...
// Sample data for past broadcasts (same as in broadcasts.ts and search-broadcasts.ts)
...
export default function handler(
  ...
  // Extract unique series names from the broadcasts data
  const uniqueSeries = Array.from(new Set(pastBroadcasts.map(broadcast => broadcast.series)));
  
  // Sort the series alphabetically for consistent ordering
  const sortedSeries = uniqueSeries.sort();
  
  // Return the unique series list
  res.status(200).json(sortedSeries);
}
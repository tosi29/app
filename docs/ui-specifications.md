...
- **仮説ボタン**: 仮説タブへ遷移（`/?tab=hypotheses&series=${series}`）
  - 💬アイコン付き「仮説を見る」テキスト
  - 該当エピソードのシリーズで自動フィルタリング
...
- **仮説ボタン**: 仮説タブへ遷移（シリーズ名正規化処理付き）
  - 検索結果のシリーズ名を正規化（例: "16-7 xxx..." → "16. xxx..."）
- **概要ボタン**: 配信概要モーダル表示（概要データがある場合のみ）
...
AI仮説自動抽出・可視化機能により、エピソード別の仮説データをUMAP次元削減による2次元散布図で可視化します。BERTopicによるトピック分類を活用し、Plotly.js標準legendによるインタラクティブな表示制御を提供します。

### レイアウト構成

#### 2カラムレイアウト
- **左側**: UMAP散布図（グラフエリア + Plotly.js標準legend）
- **右側**: 仮説詳細リスト
...
#### シリーズフィルター
- **ドロップダウン**: 全シリーズ or 特定シリーズ選択（数値順ソート対応）
- **URLパラメータ対応**: `series`指定（URLエンコード対応）
- **後方互換性**: `episodeId`パラメータも一時的にサポート（検索結果用）
- **位置**: 上部フィルターエリア（単一フィルター）

#### トピックフィルター
- **Plotly.js標準legend**: 標準legendによる表示制御（数値順ソート対応）
- **Outlierトピック**: -1.で始まるトピックは薄いグレー（#9ca3af）で表示
...
- **シリーズ指定**: `/?tab=hypotheses&series={seriesName}`
  - 特定シリーズの仮説表示（URLエンコード対応）
...
*この仕様書は現在のソースコードベース（2025年7月）に基づいて作成されています。仮説タブのPlotly.js移行、2カラムレイアウト、標準legend機能、外部API統合、Playwright MCPツールでの動作確認などの最新機能を反映しています。*